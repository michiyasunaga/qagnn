...
class QAGNN_Message_Passing(nn.Module):
    ...
    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra):
        ...
        for _ in range(self.k):
            _X, attn_log = self.gnn_layers[_](_X, edge_index, edge_type, _node_type, _node_feature_extra)
    ...
    def forward(self, H, A, node_type, node_score, cache_output=False):
        ...
        _X, attn_logs = self.mp_helper(_X, edge_index, edge_type, _node_type, _node_feature_extra)
...
class QAGNN(nn.Module):
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False):
        ...
        gnn_output, node_attn_logs = self.gnn(gnn_input, adj, node_type_ids, node_scores)
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        adj -> edge_index_orig, edge_type_orig
            edge_index (orig -> init): list of (batch_size, num_choice) -> list of (batch_size * num_choice, ); each entry is torch.tensor(2, E(variable))
                                                         -> (2, total E)
            edge_type  (orig -> init): list of (batch_size, num_choice) -> list of (batch_size * num_choice, ); each entry is torch.tensor(E(variable), )
                                                         -> (total E, )
        returns: (batch_size, 1)
        """
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index_init, edge_type_init = _inputs
        edge_index, edge_type = self.batch_graph(edge_index_init, edge_type_init, concept_ids.size(1))
        ...
        logits, node_attn_logs = self.decoder(sent_vecs.to(node_type_ids.device),
                                    concept_ids,
                                    node_type_ids, node_scores, adj_lengths, adj,
                                    emb_data=None, cache_output=cache_output)
        logits = logits.view(bs, nc)
        node_attn_logs = self.organize_node_attn_logs(node_attn_logs, edge_index_init, edge_index, bs, nc, concept_ids.size(1))
        if not detail:
            return logits, _
        else:
            return logits, node_attn_logs, concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
            ...
            #node_attn_logs: list of (batch_size, num_choice, n_layers). each entry is torch.tensor(E(variable), heads)
    ...
    def organize_node_attn_logs(self, node_attn_logs, edge_index_init, edge_index_batched, batch_size, num_choice, n_nodes):
        """
        node_attn_logs:  list of (#GNN_layer, ). each entry is alpha i.e. torch.tensor(batched_E, heads)
        edge_index_init: list of (n_examples, ). each entry is torch.tensor(2, E(variable))
        edge_index_batched: torch.tensor(2, batched_E)

        output: list of (n_examples, n_layers). each entry is torch.tensor(E(variable), heads)
            then reshaped to list of (batch_size, num_choice, n_layers)
        """
        n_examples = batch_size * num_choice
        n_layers = len(node_attn_logs)
        assert n_examples == len(edge_index_init)
        output = [[None for _ in range(n_layers)] for _ in range(n_examples)] #list of (n_examples, n_layers). each entry is torch.tensor(E(variable), heads)
        for layer_idx, alpha in enumerate(node_attn_logs):
            assert alpha.size(0) == edge_index_batched.size(1) #alpha: torch.tensor(batched_E, heads)
            global_count = 0
            for ex_idx, _edge_index in enumerate(edge_index_init): #n_examples
                local_E = _edge_index.size(1)
                _alpha = alpha[global_count : global_count+local_E] #torch.tensor(E(variable), heads)
                output[ex_idx][layer_idx] = _alpha
                global_count += local_E
            assert global_count == alpha.size(0), (global_count, alpha.size(0))
        output = list(map(list, zip(*(iter(output),) * num_choice))) #list of (batch_size, num_choice, n_layers). each entry is torch.tensor(E(variable), heads)
        assert len(output) == batch_size
        assert len(output[0]) == num_choice
        assert len(output[0][0]) == n_layers
        assert output[0][0][0].dim() == 2
        return output
...
class GATConvE(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra, return_attention_weights=True):
        ...
        orig_num_edges = edge_index.size(1)
        ...
        if return_attention_weights:
            ...
            alpha = alpha[:orig_num_edges] #[E, heads]
            return out, alpha
        else:
            return out
