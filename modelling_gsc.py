...
class GSCLayer(MessagePassing):
    ...
    def message(self, x_j, edge_attr):
        return x_j + edge_attr #add embedding of the note to the embedding of the edge
...
class GSC_Message_Passing(nn.Module):
    ...
    def edge_embeddings(self, edge_type, edge_idx, node_type_idxs):
        ...
        tail_type = node_type[edge_idx[1]]  # [E,] #tail=tg

        idx1 = self.n_ntype*head_type + tail_type
        edge_to_idx = self.n_etype*idx1 + edge_type
        edge_embeddings = self.edge_encoder(edge_to_idx)  # [E+N, emb_dim]
...
class QAGNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.gnn = GSC_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
                                         hidden_size=concept_dim, dropout=p_gnn)
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        logits = self.decoder(sent_vecs.to(node_type_ids.device),
                                    concept_ids,
