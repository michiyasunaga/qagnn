...
def main():
    ...
    parser.add_argument('--mode', default='train', choices=['train', 'eval_detail', 'print_KG'], help='run training or evaluation')
...
def print_KG(args):
    ...
    with open('data/cpnet/concept.txt', 'r', encoding='utf-8') as fin:
        id2concept = [w.strip() for w in fin]

    def get_KG(cids, ntype, edges, etype, label, attn):
        """
        cids:    torch.tensor(num_choice, n_nodes)
        ntype:   torch.tensor(num_choice, n_nodes)
        edges:   list of (num_choice, ). each entry is torch.tensor(2, E)
        etype:   list of (num_choice, ). each entry is torch.tensor(E, )
        attn:    list of (num_choice, n_layers). each entry is torch.tensor(E, heads)
        """
        n_nodes = cids.size(1)
        ret_outputs = ['KG:']
        Q_nodes = set()
        A_nodes = set()
        all_node_names = set()
        ret_edges = defaultdict(list)
        if True:
            choice_id = label
            _cids = cids[choice_id]
            _ntype = ntype[choice_id]
            for nt, cid in zip(_ntype, _cids):
                if nt==0:
                    Q_nodes.add(int(cid))
                elif nt==1:
                    A_nodes.add(int(cid))
                if int(cid) - 1 > 0:
                    all_node_names.add(id2concept[int(cid)-1])
            for e_idx, (edge, type) in enumerate(zip(edges[choice_id].t(), etype[choice_id])):
                attn_weight = '{:.4f}'.format(np.array([float(alpha[e_idx].mean()) for alpha in attn[choice_id]]).mean()) #mean of all layers
                src_nodeid, tgt_nodeid = edge
                src_cid = int(_cids[src_nodeid])
                tgt_cid = int(_cids[tgt_nodeid])
                if 2 <= type < 19:
                    rid = type - 2
                    rname = merged_relations[rid]
                elif 21 <= type < 38:
                    rid = type - 2 - 19
                    rname = merged_relations[rid] + '-1'
                else:
                    rname = 'z_link'
                srcname = id2concept[src_cid-1] if src_cid-1 > 0 else "QAcontext"
                tgtname = id2concept[tgt_cid-1] if tgt_cid-1 > 0 else "QAcontext"
                ret_edges['{}\t\t{}'.format(srcname, tgtname)].append((rname, attn_weight))
            final_edges = defaultdict(float)
            for elm in ret_edges:
                srcname, tgtname = elm.split('\t\t')
                #max pooling
                attn_weight = max([float(_edge[1]) for _edge in ret_edges[elm]])
                final_edges[elm] = attn_weight
            for elm in ret_edges:
                srcname, tgtname = elm.split('\t\t')
                _max = final_edges[elm]
                for name in tgtname.split('_') + [tgtname]:
                    key = '{}\t\t{}'.format(srcname, name)
                    if key in ret_edges:
                        _max = max(_max, final_edges[key])
                for name in tgtname.split('_') + [tgtname]:
                    key = '{}\t\t{}'.format(srcname, name)
                    if key in ret_edges:
                        final_edges[key] = _max
            ret_edges = ['{}\t\t{:.4f}'.format(elm, final_edges[elm]) for elm in final_edges]
        Q_ents, A_ents = set(), set()
        for cid in Q_nodes:
            Q_ents.add(id2concept[cid-1])
        for Q_ent in Q_ents:
            ret_outputs.append('{:5} {}'.format('Q', Q_ent))
        for cid in A_nodes:
            A_ents.add(id2concept[cid-1])
        for A_ent in A_ents:
            ret_outputs.append('{:5} {}'.format('A', A_ent))
        ret_outputs += list(ret_edges)
        return ret_outputs, Q_nodes, A_nodes, Q_ents, A_ents
    ...
    if True:
        ...
        count = 0
        dt = datetime.datetime.today().strftime('%Y%m%d%H%M%S')
        preds_path = os.path.join(args.save_dir, 'dev_preds_{}.txt'.format(dt))
        with open(preds_path, 'w') as f_preds:
            with torch.no_grad():
                for qids, labels, *input_data in tqdm(eval_set):
                    count += 1
                    logits, attns, concept_ids, node_type_ids, edge_index, edge_type = model(*input_data, detail=True)
                    predictions = logits.argmax(1) #[bsize, ]
                    preds_ranked = (-logits).argsort(1) #[bsize, n_choices]
                    for i, (qid, label, pred, _preds_ranked, cids, ntype, edges, etype, attn) in enumerate(zip(qids, labels, predictions, preds_ranked, concept_ids, node_type_ids, edge_index, edge_type, attns)):
                        if qid not in {'b67971747e95ba425a5b81e0ba8d0b28', '2ffa3808ce26181926990b454e429c85'}:
                            continue
                        outputs = []
                        outputs.append('*' * 60)
                        outputs.append('id: {}'.format(qid))
                        Q_sent = statement_dic[qid]['question']
                        outputs.append('question: {}'.format(Q_sent))
                        outputs.append('answer: {}'.format(statement_dic[qid]['answers'][label.item()].strip().replace(' ', '_')))
                        for j, answer in enumerate(statement_dic[qid]['answers']):
                            outputs.append('[{}. {}]'.format(chr(ord('A') + j), answer))
                        KG_print, Q_nodes, A_nodes, Q_ents, A_ents = get_KG(cids, ntype, edges, etype, label.item(), attn)
                        outputs += KG_print
                        outputs.append('')
                        for line in outputs:
                            f_preds.write(line + '\n')
